...
def cross_selective_scan(
    ...
    y = y.transpose(dim0=1, dim1=2).contiguous().to(x.dtype)
...
class EnhancedBlock(nn.Module):
    def __init__(self, in_channels, out_channels, drop_path_rate=0.0, expand_ratio=4, se_ratio=0.125,
                 ssm_params=None, use_checkpoint=False, **kwargs):
        ...
        self.conv_adjust = nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=1, bias=False)
        self.norm = nn.BatchNorm2d(out_channels)

        self.inverted_residual = InvertedResidual(
...
class EfficientVSSM_2(nn.Module):
    def __init__(self, patch_size=4, in_chans=3, num_classes=1000, depths=[2, 2, 9, 2], dims=[96, 192, 384, 768], drop_path_rate=0.1, **kwargs):
        super().__init__()
        self.num_classes = num_classes
        self.num_layers = len(depths)
        if isinstance(dims, int):
        ...
        for i_layer in range(self.num_layers):
            ...
            for i_block in range(depths[i_layer]):
                in_channels = self.dims[i_layer]
                out_channels = self.dims[i_layer]
                ssm_params = {
                    'ssm_d_state': 16,
                    'ssm_ratio': 2.0,
                    'ssm_rank_ratio': 2.0,
                ...
                layer.add_module(f'block_{i_block}', EnhancedBlock(
                    in_channels=in_channels,
                    out_channels=out_channels,
                    drop_path_rate=dpr[sum(depths[:i_layer]) + i_block],
                    ssm_params=ssm_params,
                    use_checkpoint=False
                ))
            if i_layer < self.num_layers - 1:
                layer.add_module(f'downsample', PatchMerging2D(self.dims[i_layer], self.dims[i_layer + 1], nn.LayerNorm))
...
def EfficientMamba_T(pretrained=False, **kwargs):
    model = Backbone_EfficientVSSM(dims=48, depths=[2, 2, 4, 2], drop_path_rate=0.2, downsample_version='v1', **kwargs)
    return model


def EfficientMamba_S(pretrained=False, **kwargs):
    model = Backbone_EfficientVSSM(dims=96, depths=[2, 2, 4, 2], drop_path_rate=0.2, downsample_version='v1', **kwargs)
    return model


def EfficientMamba_B(pretrained=False, **kwargs):
    model = Backbone_EfficientVSSM(dims=96, depths=[2, 2, 9, 2], drop_path_rate=0.2, downsample_version='v1', **kwargs)
    return model


if __name__ == "__main__":
    init = torch.randn(1, 3, 640, 640).cuda()
    models = EfficientMamba_T().cuda()
    out = models(init)
    for i in out:
        print(i.shape)
